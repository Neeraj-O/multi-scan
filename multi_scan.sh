...
function show_header() {
    clear
    echo -ne "\e[1;34mInitializing Multi Scan"
    for i in {1..3}; do
        sleep 0.5
        echo -n "."
    done
    ...
    echo -e "${CYAN}"
    figlet -c " MULTI SCAN " | while IFS= read -r line; do
        echo "$line"
        sleep 0.08
    done
    echo -e "${RESET}"
...
while true; do
    ...
    case $opt in
        ...
        16) read -p "Enter target: " target; run_scan "nmap -sC -sV -O -Pn $target" "$target" ;;
        ...
        23) read -p "Enter target: " target; run_scan "nmap -p 80,443 --script http-waf-detect,http-waf-fingerprint $target" "$target" ;;
        ...
        26) read -p "Enter target: " target; run_scan "nmap --script whois-domain --script-args whois-domain $target" "$target" ;;
